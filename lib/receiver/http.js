...
class Receiver {
			...
			if (req.method === 'POST') {
				...
				form.parse(req, function (err, fields, files) {
					...
					try {
						...
						fields.path = JSON.parse(fields.path);
	...
	process(fields, files, cb) {
		...
		switch (fields.command) {
			...
			case 'mkdirs':
			case 'mkdir':
				fs.mkdirs(fields.path.src, function (err) {
					if (err) return cb(err);
					
					me.utimes(fields.path.src, fields.stats.atime, fields.stats.mtime, cb);
				});
				break;
			case 'write':
			case 'writeFile':
			case 'createWriteStream':
				if (files.file) {
					fs.mkdirs(path.dirname(fields.path.src), (err) => {
						if (err) return cb(err);
						
						// Перемещаем файл из временной директории в нужное место
						fs.move(files.file.path, fields.path.src, {clobber: true}, function (err) {
							if (err) return cb(err);
					...
					cb(new exceptions.ErrorReceiverProcess(`Not exist file for path "${fields.path.src}"`, fields));
				}
				break;
			case 'rmdir':
				fs.rmdir(fields.path.src, cb);
				break;
			case 'unlink':
			case 'remove':
			...
			case 'symlink':
				...
				break;
			default:
				cb(new exceptions.ErrorReceiverProcess(`"${fields.command}" - command not found, for path: ${fields.path.src}`, fields));
		}
	}
