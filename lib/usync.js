...
class USync {
	...
	connectToRabbitMq() {
		const me = this;
		me.rbmqConnection = me.amqp.createConnection(me.config.rabbitmq);
					...
					(letter, cb) => {
						async.each(
							...
							(transmitter, cb) => {
								...
								me.rbmqConnection.queue(transmitter.queuePrefix + '_' + letter, {
									autoDelete: false,
									durable   : true
								}, (queue) => {
									me.possibleQueues[transmitter.queuePrefix + '_' + letter] = 1;
									queue.close();
									cb();
								});
							},
						...
						async.each(
							...
							(message, cb) => {
								...
								let timeOutId = setTimeout(() => {
									isRunCb = true;
									me.fireEventError(new me.exceptions.TimeoutPublishMesssage(task, me.config.timeoutPublishMessage));
									cb();
								}, me.config.timeoutPublishMessage);
								
								
								me.debug(task, `${task.command}(${task.id}) push begin ${message.queueName} | ${task.getTimeElapsed()}ms | ${task.getTimeElapsedStorage()}ms`);
								...
								me.exchange.publish(
									...
									(result) => {
										...
										// Если колбек был вызван ранее
										if (isRunCb) return;
										
										if (result) {
											me.fireEventError(new exceptions.ErrorTask(task, 'Message not send'));
										} else {
											me.debug(task, `${task.command}(${task.id}) push end ${message.queueName} | ${task.getTimeElapsed()}ms `);
										}
