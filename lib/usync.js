			...
			setTimeout(() => {
				async.each(
					...
					(message, cb) => {
						...
						me.debug(task, `${task.command}(${task.id}) save in storage begin ${message.queueName} | ${(new Date().getTime() - task.dateCreate.getTime())}ms`);
						
						//Создаем копию файла(чтобы иметь его состояние в момент выполнения операции)
						me.saveInStorage(message, (err) => {
							if (err) {
								// На случай, если при записи "копии файла" фозникла ошибка, возвращать ошибку нельзя!
