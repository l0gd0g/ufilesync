	...
	connectToRabbitMq() {
		...
		amqplib.connect(me.config.rabbitmq.connectionConfig, function (err, connection) {
			...
			me.rbmqConnection.createChannel((err, channel) => {
				...
				// me.channel.on('return', task => {
				// });
				//
				// me.channel.on('delivery', function() {
				// });
				
				// channel.close();//TODO проверить закрываются ли каналы самостоятельно рабитом, иначе в деструкторе закрывать самому
				me.prepareQueue(() => {
					me.isReadyPush = true;
					me.isConnected = true;
					me.fireEventReady();
