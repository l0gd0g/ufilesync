...
class USync {
	...
	constructor(_config) {
		...
		if (!me.config.regExpFindDirs) {
			me.config.regExpFindDirs = new RegExp(`.*(${me.config.watchDirs.join('|')})/`, 'i');
		}
		// Флаг показывает, готовы ли мы сейчас отправлять задачи в очередь
		me.isReadyPush = false;
		
		if (!me.config.regExpFindPathStorage) {
			me.config.regExpFindPathStorage = new RegExp(`^((${this.config.watchDirs.join('|')})/\\w\\/\\w\\/\\w\\/)(.*)$`);
		}
	...
	restartChannelRabbitMq(cb) {
		...
		// Так как тварь channel может уже отвалиться и объект не будет работать(но будет в памяти), если к нему обратиться, упадет с ошибкой
		try {
	...
	task(command, path, subject, debug) {
		if (!path || command.toString().length === 0) {
			return new this.exceptions.ErrorCreateTask('Param command required');
		}
		if (!command || path.toString().length === 0) {
			return new this.exceptions.ErrorCreateTask('Param path required');
		}
		
		if (this.config.isRunDebugMode) {
			if (this.config.debugCommands.includes(command)) {
				debug = true;
			} else {
				debug = false;
			}
		}
		const queueNamePostfix = this.selectQueuePostfix(path.dest || path);
		...
		try {
			...
			const errUSync = new this.exceptions.ErrorUSync(err.message);
					...
					err => {
						...
						async.each(
							...
							(message, cb) => {
								...
								const send = function (cb) {
									// Так как тварь channel может уже отвалиться и объект не будет работать(но будет в памяти), если к нему обратиться, упадет с ошибкой
									try {
										me.channel.sendToQueue(
											message.queueName,
	...
	saveInStorage(message, cb) {
		if (this.config.fileSendMethods.includes(message.command)) {
