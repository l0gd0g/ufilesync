	...
	wrapOriginCb(arg, fsMe, task) {
		...
		arg.push(function () {// подменяем на свой
			...
			if (argCb[0]) {
				...
				argCb[0].stack = stackPlace.stack;
				...
				decorator.uSync.push(task, err => {
					if (err) {
						...
						decorator.uSync.debug(task, `${decorator.functionName}(${task.id}) push end | ${task.getTimeElapsed()}ms | ${task.getTimeElapsedPush()}ms | ${JSON.stringify(task.path)}`);
