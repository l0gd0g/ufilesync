						...
						return (cb) => {
							...
							fs.lstat(path, function (err, stats) {
								...
								if (stats.isDirectory()) {
									...
									decorator.uSync.push(decorator.uSync.task('mkdirp', path, null, 'copy recursive'), function (err) {
										if (err) return cb(err);
									...
									fs.readlink(path, function (err, linkPath) {
										...
										decorator.uSync.push(decorator.uSync.task('symlink', {
											src : linkPath,
											dest: path
										}, null, 'copy symlink'), cb);
									});
								} else {
									// Отправляем в очередь на синхронизацию
									decorator.uSync.push(decorator.uSync.task('writeFile', path, null, 'copy recursive'), cb);
								}
							});
						}
					}),
