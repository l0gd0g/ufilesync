...
class Decorator {
	...
	generateTaskFromArguments(arg) {
		...
		if (arg[0] instanceof decorator.uSync.tasks.Task) {
			...
			let queueName = decorator.uSync.selectQueueName(pathToFiles);
						...
						return (cb) => {
							...
							fs.lstat(path, function (err, stats) {
								...
								if (stats.isDirectory()) {
									...
									decorator.uSync.push(decorator.uSync.task('mkdirp', path, null, 'copy recursive'), 0, function (err) {
										if (err) return cb(err);
									...
									fs.readlink(path, function (err, linkPath) {
										...
										decorator.uSync.push(decorator.uSync.task('symlink', {
											src : linkPath,
											dest: path
										}, null, 'copy symlink'), 0, cb);
									});
								} else {
									// Отправляем в очередь на синхронизацию
									decorator.uSync.push(decorator.uSync.task('writeFile', path, null, 'copy recursive'), 0, cb);
								}
							});
						}
					}),
	...
	wrapOriginCb(arg, fsMe, task) {
		...
		arg.push(function () {// подменяем на свой
			...
			if (argCb[0]) {
				decorator.uSync.debug(task, `${decorator.functionName}(${task.id})  error | ${(new Date().getTime() - task.dateCreate.getTime())}ms | ${JSON.stringify(task.message.path)} | ${argCb[0]}`);
	...
	wrap(originFunction) {
		...
		return function () {
			...
			decorator.uSync.on('ready', () => {
				decorator.uSync.debug(task, `${decorator.functionName}(${task.id}) begin | ${(new Date().getTime() - task.dateCreate.getTime())}ms | ${JSON.stringify(task.message.path)}`);
