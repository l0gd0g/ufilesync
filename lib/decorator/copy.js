...
class DecoratorCopy extends Decorator {
	...
	wrapOriginCb(arg, fsMe, task) {
		...
		arg.push(function () {// подменяем на свой
			...
			if (argCb[0]) {
				decorator.uSync.debug(task, `${decorator.functionName}(${task.id}) error | ${task.getTimeElapsed()}ms | ${JSON.stringify(task.path)}`);
				
				// Вызываем cb который передал программист
				cbOrigin.apply(fsMe, argCb);
			} else {
				decorator.uSync.debug(task, `${decorator.functionName}(${task.id}) fs end | ${task.getTimeElapsed()}ms | ${task.getTimeElapsedFs()}ms | ${JSON.stringify(task.path)}`);
					...
					fs.lstat(task.path.src, function (err, stats) {
						...
						if (stats.isDirectory()) {
							decorator.generateTaskFromScanDir(task.path.dest, isReplace, function (err) {
								...
								decorator.uSync.debug(task, `${decorator.functionName}(${task.id}) push | ${task.getTimeElapsed()}ms | ${JSON.stringify(task.path)}`);
							...
							fs.lstat(task.path.dest, function (err) {
								...
								fs.readlink(task.path.src, function (err, linkPath) {
									...
									decorator.uSync.push(
										...
										function (err) {
											decorator.uSync.debug(task, `${decorator.functionName}(${task.id}) push unlink | ${task.getTimeElapsed()}ms | ${JSON.stringify(task.path)}`);
											if (err) {
												cbOrigin(err);
											} else {
												decorator.uSync.push(
													...
													function (err) {
														decorator.uSync.debug(task, `${decorator.functionName}(${task.id}) push symlink | ${task.getTimeElapsed()}ms | ${JSON.stringify(task.path)}`);
														if (err) {
															cbOrigin(err);
														} else {
															// Вызываем cb который передал программист
