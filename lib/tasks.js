...
class Task {
	constructor() {
		...
		this.timeElapsed = {
			// прошло времени на выполнение  команды библиотекой fs
			fs     : null,
			push   : null,
			storage: null,
		};
		...
		this.dates = {
			...
			// Время конца выполнения функции модулем fs
			fs: 0,
			
			// Время конца операции сохранения файлов в хранилище
			storage: 0,
			
			// Время конца отправки задачи в RabbitMq
			push: 0,
		};
	...
	/**
	 * Сколько прошло времени на выполнение задачи модулем fs
	 * @returns {number}
	 */
	getTimeElapsedFs() {
		if (this.timeElapsed.fs === null && this.dates.fs) this.timeElapsed.fs = this.dates.fs.getTime() - this.dates.begin.getTime();
		
		return this.timeElapsed.fs;
	}
	
	/**
	 * Сколько прошло времени на сохранение файлов в хранилище
	 * @returns {number}
	 */
	getTimeElapsedStorage() {
		if (this.timeElapsed.storage === null && this.dates.fs && this.dates.storage) this.timeElapsed.storage = this.dates.storage.getTime() - this.dates.fs.getTime();
		
		return this.timeElapsed.storage;
	}
	
	/**
	 * Сколько прошло времени на отправку push в RabbitMq
	 * @returns {number}
	 */
	getTimeElapsedPush() {
		if (this.timeElapsed.push === null && this.dates.push) this.timeElapsed.push = this.dates.push.getTime() - this.dates.storage.getTime() || this.dates.begin.getTime();
		
		return this.timeElapsed.push;
	}
}
