...
class Simple extends Task {
	...
	constructor(config, command, _path, queueName, subject, timeDelay, debug) {
		...
		const idPostfix = `${(new Date()).getTime()}_${Math.floor(Math.random() * 999)}`;
		
		//Решили хранить с полным путем + время + случайное число
		this.id = `${this.path.src.substr(0, MAX_TASK_NAME_LENGTH)}_${idPostfix}`;
		
		const match = path.normalize(this.path.dest || this.path.src).match(this.config.regExpFindPathStorage);
		if (match && match.length >= 3) {
			this.id = `${(match[1] + urlencode(match[3])).substr(0, MAX_TASK_NAME_LENGTH)}_${idPostfix}`;
			this.path.store = this.id;
		} else {
			this.id = `${path.normalize(this.path.dest || this.path.src).substr(0, MAX_TASK_NAME_LENGTH)}_${idPostfix}`;
		}
	...
	fillStatsIfNeed(path, cb) {
		if (this.config.fileGetStats.includes(this.command)) {
			try {
